...
dt = 0.05;% diff t
N = 5; % swarm's size
P_saclm = [7.0789 1.8849 0.01 0.00032157 0.9495]; %[ps pa pc lambda mu]
swarm = [];
goal = [80;75;40];
for i = 1:N
    swarm = [swarm Drone([randi([5,15]);randi([5,15]);40],[0;0;0],6,P_saclm,goal)];
...
for t = 1:dt:Tmax
    ...
    %     % saveas(simulation_fig,['D:\Study\导师任务\发表论文\23_11_30\runtime\cylinder\' ['运行于第' num2str(t) 's'] '.eps']);
    % end
    for i = 1:N
...
box on;
hold off;
% print('distance_between_UAVs.eps', '-depsc');
